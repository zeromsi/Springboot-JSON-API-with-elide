...
public class services {
	...
	@CrossOrigin(origins = "*")
	@RequestMapping(method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE, value = { "/{entity}",
			"/{entity}/{id}/relationships/{entity2}" })
	@Transactional
	...
	/**
	 * All our elide operations require similar initialisation, which we perform in
	 * this method before calling elideCallable with the elide object and the path
	 * that elide needs to know what it is supposed to do.
	 * 
	...
	private String elideRunner(final HttpServletRequest request, final ElideCallable elideCallable) {
		...
		/*
		 * Elide works with the Hibernate SessionFactory, not the JPA
		 * EntityManagerFactory. Fortunately we san unwrap the JPA EntityManagerFactory
		 * to get access to the Hibernate SessionFactory.
		 */
		...
		/*
		 * There is a bug in Elide on Windows that will convert a leading forward slash
		 * into a backslash, which then displays the error
